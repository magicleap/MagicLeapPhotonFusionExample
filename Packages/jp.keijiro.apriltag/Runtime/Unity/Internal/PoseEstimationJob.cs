...
namespace AprilTag
{
	...
	struct PoseEstimationJob : Unity.Jobs.IJobParallelFor
	{
		...
		public struct Input
		{
			...
			unsafe public Input(ref Interop.Detection r) => p = (Interop.Detection*)Interop.Util.AsPointer(ref r);

			unsafe public ref Interop.Detection Ref => ref Interop.Util.AsRef<Interop.Detection>(p);
		}
